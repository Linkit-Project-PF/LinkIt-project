...
export default function HowToApplyCarousel() { 
    ...
    return ( 
        <div className="">
            ...
            <div className="mt-5">
            <motion.div initial={{opacity: 0, scale: 0.5}} animate={{opacity: isActive === 1 ? 1 : 0, scale: 1}} transition={{duration: 0.3}} className={`${isActive === 1 ? 'block' : 'hidden'} space-y-3`}><h2 className="font-manrope font-bold whitespace-nowrap subtitles-size">{t('Consulta nuestras vacantes')}</h2><p className="font-montserrat font-medium text-size xs:mx-1.5" >{t('Tenemos posiciones abiertas en múltiples áreas de crecimiento.')}</p></motion.div>
            <motion.div initial={{opacity: 0, scale: 0.5}} animate={{opacity: isActive === 2 ? 1 : 0, scale: 1}} transition={{duration: 1}} className={`${isActive === 2 ? 'block' : 'hidden'} space-y-3`}><h2 className="font-manrope font-bold subtitles-size ">{t('Aplica completando el formulario')}</h2><p className="font-montserrat font-medium text-sizexs:mx-1.5 sm:mx-10  ">{t('Selecciona tu posición ideal rellenando la solicitud en pocos clicks.')}</p></motion.div>
            <motion.div initial={{opacity: 0, scale: 0.5}} animate={{opacity: isActive === 3 ? 1 : 0, scale: 1}} transition={{duration: 1}} className={`${isActive === 3 ? 'block' : 'hidden'} space-y-3`}><h2 className="font-manrope font-bold whitespace-nowrap  subtitles-size">{t('Proceso de entrevistas')}</h2><p className="font-montserrat font-medium text-sizexs:mx-1.5 sm:mx-10  ">{t('Conoce en detalle la oportunidad y prepárate para los próximos pasos.')}</p></motion.div>
            <motion.div initial={{opacity: 0, scale: 0.5}} animate={{opacity: isActive === 4 ? 1 : 0, scale: 1}} transition={{duration: 1}} className={`${isActive === 4 ? 'block' : 'hidden'} space-y-3`}><h2 className="font-manrope font-bold subtitles-size">{t('Consigue el trabajo de tus sueños')}</h2><p className="font-montserrat font-medium text-sizexs:mx-1.5 sm:mx-10  ">{t('Comienza a trabajar de forma remota y lleva tu carrera al siguiente nivel.')}</p></motion.div>
            </div>
