...
const SuperAdminComponent: FunctionComponent<ISuperAdminComponentProps> = ({
  user,
  role,
}) => {
  ...
  useEffect(() => {
    ...
    const updatedAdmin = async () => {
      try {
        const editedAdmin = await editUser({ ...user, image: filePublicId });
        dispatch(setUser(editedAdmin));
        if (editedAdmin) {
          Swal.fire({
    ...
    <div className="flex w-full mt-20 h-[22vh] justify-center">
      <div className="flex justify-between w-11/12 p-3">
        ...
        <div className="flex space-x-4 w-1/2 justify-end items-end">
          ...
          <div
            className={`relative rounded-full w-36 h-36 bg-gray-300 ${
              user.image
