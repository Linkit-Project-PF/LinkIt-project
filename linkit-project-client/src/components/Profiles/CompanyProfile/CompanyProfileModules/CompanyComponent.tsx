...
const CompanyComponent: FunctionComponent<IComponentProps> = ({company}) => {
  ...
  useEffect(()=>{
    ...
    const updatedCompany= async ()=>{
      try {
        const editedCompany = await editCompany({...company, image: filePublicId})
        dispatch(setCompany(editedCompany))
        if (editedCompany){
          Swal.fire({
            title: "Imagen actualizada",
            text: `${editedCompany.image}`,
            icon: "success",
        ...
        <div className="flex space-x-4 w-1/2 justify-end items-end">
          ...
          <div className={`relative rounded-full w-36 h-36 bg-gray-300 ${company.image ? "flex flex-col justify-center items-center content-center" : ''}`}>
            <div className="relative w-full h-full">
              ...
              {
                company.image && (
                  <img src={`https://res.cloudinary.com/dquhriqz3/image/upload/${company.image}`} alt="" className="rounded-full h-full"/>
                ) 
              }
            </div>
          </div>
